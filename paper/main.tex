...
%Our tools are implemented in an R package \ash (short for ``Adaptive Shrinkage") and we 
...
%Although we motivate our tools by the need to address the FDR problem, our \ash package, as its name suggests,  provides a general tool for shrinkage estimation. 
...
\end{equation}
%Here, in addition to some conditional independence assumptions, we are 
...
% Indeed, neither $\locfdr$ nor $\qvalue$ explicitly model the effect distribution at all:  they effectively avoid this by simply subtracting a null distribution 
%(of $z$ scores or $p$ values) from the observed empirical distribution, with the remains necessarily 
%representing the alternative distribution. When doing this they assume that all the $z$ scores near zero (or, equivalently,
%all $p$ values near 1) are null, which results in a ``hole" in the alternative distribution

%Indeed, $\locfdr$ \cite{efron2008microarrays} assumes that all $z$ scores near 0 are null 
%(which we call the Zero Assumption, \sZA), which implies that 
%under the alternative hypothesis the distribution of $z$ scores has {\it no support at 0}.  
%(Although \cite{efron2008microarrays} motivates and defines the \sZA as ``most $z$ scores near 0 are null",
%results from $\locfdr$ actually obey the stronger version of the \sZA  that we use here: {\it all} $z$ scores near 0 are null.)
%Similarly, $\qvalue$ assumes -- when estimating $\pi_0$ -- that all $p$ values near 1 are null.
%which, if the $p$ values were derived from $z$ scores, is the ZA because $p$ values near 1 correspond to $z$ scores near 0.
%And although \mixfdr does not formally make the \sZA,
%we have found that in practice, with default settings, the results often approximately satisfy this assumption (due, we believe, to the default choice of penalty term $\beta$ described in \cite{muralidharan2010empirical}).
%Tukey "peri-null"
...
%Although \mixfdr does not formally make the \sZA, its decomposition
%exhibits a similar hole. 
%In contrast, due to the UA, the alternative $z$ score distribution for \ash is required to have a mode at 0, 
%effectively ``filling in" the hole.  
...
%Interestingly, even in the ``bimodal" scenario $\ash$ 
%is more accurate than \mixfdr: although \mixfdr is capable, in principle, of fitting the multiple modes of $g$, it does not do this well here. 
...
%
%We note one caveat on the accuracy of estimated $g$: due to the penalty term (\ref{eqn:penalty}) \ash tends to systematically
%overestimate the mass of $g$ near zero. This can be seen in Figure \ref{fig:egcdf-reduce}: the estimated cdf is generally below the true cdf just to the left of zero,
%and above the true cdf just to the right of zero. Averaging the cdf over many replicates confirms this systematic effect (Figure \ref{fig:egcdf-reduce}b),
%and applying our methods without the penalty term removes this systematic effect, although at the cost of sometimes 
%under-estimating $\pi_0$ (Figure \ref{fig:egcdf-reduce}c). 
...
%MorrisÕ Section 3 is especially pertinent. His formula
%for p(?|z) is related to my discussion of the
%BenjaminiÐYekutieli False Coverage Rate method in
%Section 7, particularly (7.2)Ð(7.4). Originally I had
...
%Section 6 on the Òone-group modelÓ is the ugly duckling
...
%where, as in MorrisÕ hospital example, the usual singlepoint
...
%Rice and SpiegelhalterÕs penultimate question.
...
%models like (7.1) are to deal with. JamesÐStein estimation
%works fine with, say, N = 10 component problems,
%while the RobbinsÕ form of empirical Bayes appropriate
...
\begin{subtable}{\textwidth}
	\centering\input{../analysis/table/scoverage_all.tex}
...
\begin{subtable}{\textwidth}
\centering\input{../analysis/table/scoverage_neg.tex}
...
\begin{subtable}{\textwidth}
\centering\input{../analysis/table/scoverage_pos.tex}
...
\centering\includegraphics[width=3in]{../analysis/figure/make_GOODPOOR_figs.Rmd/lfsr_vs_pval_GOODPOOR_single-2.pdf}
    \caption{The relationship between $\lfsr$ and $p$-value is different for good-precision ($s=1$) and low-precision ($s=10$) measurements: \ash assigns the low-precision measurements a higher $\lfsr$, effectively downweighting them.    %The default prior produces better performance here, as expected because the simulations match its assumptions. In practice one \ash can assess which prior best matches the data by comparing likelihoods (see text).
    } \label{fig:lfsr_pval}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\centering\includegraphics[width=3in]{../analysis/figure/make_GOODPOOR_figs.Rmd/tp-fp-1.pdf}
    ...
    %The default prior produces better performance here, as expected because the simulations match its assumptions. In practice one \ash can assess which prior best matches the data by comparing likelihoods (see text).
    } \label{fig:tpfp}
\end{subfigure}
...
%(Note that results above that were generated with all $\shat_j=1$ would not be affected by choice of $\alpha$.)
...
%Framing the problem in this way -- that is, as comparing different modelling assumptions for $\beta_j$, rather than as comparing ``modelling $\beta_j$" vs  ``modelling $z_j$" (or ``modelling $p_j$") -- has the important advantage that 
%likelihood-based methods can be used to select $\alpha$. For example, 



% He named this prior the ``$p$-value prior" because it can be thought of as the 
%implicit prior assumption that is made when using $p$ values to rank observations.
%Wakefield's $p$-value prior assumes that the prior for $\beta_j$ depends on its standard error $s_j$,
%and specifically that 
%\begin{equation}
%\beta_j | s_j \sim N(0,K s^2_j) \text{for some constant $K$.}
%\end{equation}
%Here we observe that Wakefield's result extends to our mixture of (zero-mean) normal priors. 
%Specifically, if, instead of assuming that $\beta_j$ is independent
...
%then the $\lfsr$ provides the {\it same ranking} of observations as the $z$ scores and $p$ values,
%as is illustrated in Figure \ref{fig:lfsr_pval}, right panel. 
...
%To summarize: the behaviour of \ash depends on the value of $\alpha$ assumed in (\ref{eqn:beta-alpha}). 
...
%\begin{figure}
%\begin{subfigure}{\textwidth}
%\centering\centering\includegraphics[width=4in]{../analysis/figure/make_GOODPOOR_figs.Rmd/lfsr_vs_pval_GOODPOOR-1.pdf} \label{fig:lfsr_pval}
%    \caption{Illustration of how prior assumptions re-order significance. Left panel shows results under the ``default prior", which assumes $\beta_j$ are identically distributed, independent of $s_j$ ($\alpha=0$ in (\ref{eqn:beta-alpha})). Right panel shows results under the ``$p$-value prior", which assumes $z$ scores $\beta_j/s_j$ are identically distributed, independent of $s_j$ (\ref{eqn:beta-alpha}). This right panel is exactly the same as would be obtained by any monotonic function of the $p$ values to measure significance (e.g. $p$ values, $q$ values, $|z|$ scores)}
% \end{subfigure}
%
%\caption{} 
%\end{figure}
...
\begin{table}[!ht]
\begin{subtable}{\textwidth}
	\centering\input{../analysis/table/scoverage_all_nopen.tex}
	\caption{All observations}
\end{subtable}

\begin{subtable}{\textwidth}
\centering\input{../analysis/table/scoverage_neg_nopen.tex}
\caption{``Significant" negative discoveries.}
\end{subtable}

\begin{subtable}{\textwidth}
\centering\input{../analysis/table/scoverage_pos_nopen.tex}
...
\end{table}
%In simple settings (e.g. symmetric effects, with the same standard error) $\lfdr_j$ is a monotonic function of the $p$ value $p_j$, and so
%\begin{equation}
