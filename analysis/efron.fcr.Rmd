...
```{r}
  ...
  par(mfcol=c(2,1))
  nullalthist(z,lfdr=ashr::get_lfdr(res.ash.true),main="true partition")
  nullalthist(z,lfdr=ashr::get_lfdr(res.ash),main="inferred partition")
```
...
```{r}
  par(mfcol=c(1,1))
  plot(ashr::get_pm(res.ash.true),ashr::get_pm(res.ash),xlab="Truth", ylab="ash.hu",main="Posterior Mean (inferred vs truth)",xlim=c(-6,1),ylim=c(-6,1))
  abline(a=0,b=1,col=2,lwd=3)

    plot(ashr::get_lfdr(res.ash.true),ashr::get_lfdr(res.ash),xlab="Truth", ylab="ash.hu",main="lfdr (inferred vs truth)",xlim=c(0,1),ylim=c(0,1))
  abline(a=0,b=1,col=2,lwd=3)

  plot(ashr::get_lfsr(res.ash.true),ashr::get_lfsr(res.ash),xlab="Truth", ylab="ash.hu",main="lfsr (inferred vs truth)",xlim=c(0,1),ylim=c(0,1))
  abline(a=0,b=1,col=2,lwd=3)
...
par(mfrow=c(1,3))
plot(ashr::get_lfdr(res.ash.true),ashr::get_lfdr(res.ash),xlab="Truth (lfdr)", ylab="ash.hu",main="ash.hu",xlim=c(0,1),ylim=c(0,1))
abline(a=0,b=1,col=2,lwd=3)
plot(ashr::get_lfdr(res.ash.true),res.locfdr$fdr,xlab="Truth (lfdr)", ylab="Estimate",main="locfdr",xlim=c(0,1),ylim=c(0,1))
abline(a=0,b=1,col=2,lwd=3)
plot(ashr::get_lfdr(res.ash.true),res.qvalue$lfdr,xlab="Truth (lfdr)", ylab="Estimate",main="qvalue",xlim=c(0,1),ylim=c(0,1))
abline(a=0,b=1,col=2,lwd=3)
...
```{r}
  ...
  BayesComparePlot=function(CImatrix,altmean=-3,...){
    ...
    points(z,CImatrix[,1],col="red")
    points(z,CImatrix[,2],col="red")
