...
The referee reports a simulation of 10,000 hypotheses, a fraction 0.8 are null ($\mu_i=0$), and a fraction 0.2 are alternative with effect $\mu_i \sim N(0,1)$. For each effect $i$ there
...
unimodal assumption, and ii) have the property that "most" $z$ scores near 0 are null.

Here we repeat these simulations and apply qvalue to check how it behaves.
...
```{r}
lfdr.ash=ashr::ash(x.est,1/sqrt(30))$lfdr
...
lfdr.true=ashr::ash(x.est,1/sqrt(30),g=trueg,fixg=TRUE)$lfdr
