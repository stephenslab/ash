...
scenario are shrunk more strongly, illustrating that shrinkage adapts to the underlying distribution of beta; ii) in both cases, estimates with large
...
make_df_for_ashplot =
  function (sim1, sim2, ash1, ash2, name1 = "spiky", name2 = "big-normal") {
    ...
    lfsr = c(get_lfsr(ash1),get_lfsr(ash2))
...
ashplot = function(df,xlab="Observed beta-hat",ylab="Shrunken beta estimate") {
  ggplot(df,aes(x = betahat,y = beta_est,color = 1/s)) +
    xlab(xlab) + ylab(ylab) + geom_point() + facet_grid(.~scenario) +   geom_abline(intercept = 0,slope = 1,linetype = "dotted") +
    scale_colour_gradient2(midpoint = median(1/s),low = "blue",mid = "white",
                           high = "red",space = "Lab") +
    coord_fixed(ratio = 1)
 }
...
```{r run_ash_ET}
beta.bignormal.ash.ET = ash(sim.bignormal$input$betahat, s,alpha=1,mixcompdist="normal")
beta.spiky.ash.ET = ash(sim.spiky$input$betahat, s,alpha=1,mixcompdist="normal")
