...
res = rbind(data.frame(x= res.qvalue.good$qvalues, y = res.qvalue$qvalues[GOOD],type="qvalue"),
            ...
            data.frame(x=get_lfsr(res.ash.good),y=get_lfsr(res.ash)[GOOD],type="ashr"))
...
make_df_for_pval=function(ash,method="default"){
  df = data.frame(p=pnorm(-abs(ash$data$x/ash$data$s)),lfsr=get_lfsr(ash),s=ash$data$s,method=method)
}
...
res.ash.ET = ash(betahat,sebetahat,method="fdr",alpha=1,mixcompdist="normal",outputlevel=4)
